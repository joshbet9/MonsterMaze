"""Entry point for the Monster Maze Discord bot and results API."""
import asyncio
import os
...
def refresh_bot(platform):
    ...
    try:
        ...
        future = asyncio.run_coroutine_threadsafe(bot.refresh_competitions(), bot.loop)
        future.result(timeout=10)
...
async def _post_feed_async(bot, run):
    ...
    embed = discord.Embed(title=f"{run['name']} - Solo Run (stage {run['stage']})", color=0x33AA66)
...
class APIMonsterBot(tournament_impl.TournamentBot):
...
def configure_api():
    api_server.configure(
        db_fn=base.db,
        insert_submission=base.insert_submission,
        upsert_run=base.upsert_run,
        create_competition=lambda platform: base.create_competition(platform, base.load_config()),
        board_rows=base.board_rows,
        competition_rows=base.competition_rows,
...
def main():
    ...
    APIMonsterBot(base.load_config()).run(base.load_config()["token"])
