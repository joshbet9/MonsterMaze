...
def create_tournament(c, season_id, number, name, registration_start, registration_end, start_ts):
    ensure_schema(c)
    cur = c.execute("INSERT INTO tournaments(season_id,number,name,registration_start,registration_end,start_ts,status) VALUES(?,?,?,?,?,?, 'registration')", (season_id,number,name,registration_start,registration_end,start_ts))
    c.commit(); return int(cur.lastrowid)


def register(c, tournament_id, uuid, name):
    ensure_schema(c)
    t = c.execute("SELECT season_id,status,registration_end FROM tournaments WHERE id=?",(tournament_id,)).fetchone()
    ...
    u=uuid.lower()
    c.execute("INSERT OR IGNORE INTO season_players(season_id,uuid,name) VALUES(?,?,?)",(int(t[0]),u,name))
    c.execute("INSERT OR IGNORE INTO tournament_players(tournament_id,uuid,name,registered_at) VALUES(?,?,?,?)",(tournament_id,u,name,now))
    c.commit()
...
def _advance(c, tournament_match_id, winner_uuid):
    ...
    col="player1_uuid" if int(row[1])==1 else "player2_uuid"
    c.execute("UPDATE tournament_matches SET %s=? WHERE id=?"%col,(winner_uuid.lower(),int(row[0])))
    _activate_if_ready(c,int(row[0]))
...
def build_bracket(c,tournament_id):
    ...
    for rnd in range(1,rounds):
        for slot in range(1,size//(2**rnd)+1):
            nxt=ids[(rnd+1,(slot+1)//2)];target=1 if slot%2 else 2
            c.execute("UPDATE tournament_matches SET next_match_id=?,next_slot=? WHERE id=?",(nxt,target,ids[(rnd,slot)]))
    first=size//2
    for slot in range(1,first+1):
        i=(slot-1)*2;p1=seeded[i][0] if i<len(seeded) else None;p2=seeded[i+1][0] if i+1<len(seeded) else None
    ...
    if rounds>=2:
        ...
        c.execute("UPDATE tournament_matches SET next_match_id=?,next_slot=? WHERE id=?",(third,1,ids[(rounds-1,1)]))
        c.execute("UPDATE tournament_matches SET next_match_id=?,next_slot=? WHERE id=?",(third,2,ids[(rounds-1,2)]))
...
def current_match(c,tournament_id,uuid):
    ensure_schema(c);u=uuid.lower()
    row=c.execute("SELECT id,round_number,slot,player1_uuid,player2_uuid,best_of,player1_wins,player2_wins,status FROM tournament_matches WHERE tournament_id=? AND status IN ('ready','active') AND (player1_uuid=? OR player2_uuid=?) ORDER BY round_number ASC,slot ASC LIMIT 1",(tournament_id,u,u)).fetchone()
...
def record_game(c,tournament_match_id,game_number,platform,mode,pattern,kit,match_id,winner_uuid):
    ensure_schema(c)
    row=c.execute("SELECT tournament_id,player1_uuid,player2_uuid,status,player1_wins,player2_wins,round_number FROM tournament_matches WHERE id=?",(tournament_match_id,)).fetchone()
    ...
    p1w,p2w=int(row[4]),int(row[5]);p1w+=winner==row[1];p2w+=winner==row[2]
    c.execute("UPDATE tournament_matches SET status='active',player1_wins=?,player2_wins=? WHERE id=?",(p1w,p2w,tournament_match_id))
    if p1w>=2 or p2w>=2:
        champ=row[1] if p1w>=2 else row[2];loser=row[2] if champ==row[1] else row[1]
        c.execute("UPDATE tournament_matches SET status='complete',winner_uuid=?,loser_uuid=?,completed_at=? WHERE id=?",(champ,loser,int(time.time()*1000),tournament_match_id));_advance(c,tournament_match_id,champ)
        rounds=c.execute("SELECT bracket_size FROM tournaments WHERE id=?",(row[0],)).fetchone()[0];final_round=rounds.bit_length()-1
        if int(row[6])==final_round-1 and final_round>=2:
            ...
            if third:
                c.execute("UPDATE tournament_matches SET player1_uuid=? WHERE id=? AND player1_uuid IS NULL",(loser,third[0]));c.execute("UPDATE tournament_matches SET player2_uuid=? WHERE id=? AND player2_uuid IS NULL",(loser,third[0]));_activate_if_ready(c,third[0])
...
def finalize(c,tournament_id):
    ...
    rounds=int(t[2]).bit_length()-1
    final=c.execute("SELECT winner_uuid,loser_uuid,status FROM tournament_matches WHERE tournament_id=? AND round_number=?",(tournament_id,rounds)).fetchone()
    if not final or final[2]!='complete':raise ValueError("final is not complete")
    champion,runner=final[0],final[1]
    ...
    c.execute("UPDATE tournament_players SET placement=1 WHERE tournament_id=? AND uuid=?",(tournament_id,champion));c.execute("UPDATE tournament_players SET placement=2 WHERE tournament_id=? AND uuid=?",(tournament_id,runner))
    if third and third[2]=='complete':
