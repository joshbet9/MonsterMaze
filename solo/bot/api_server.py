...
def competitive_get(parts):
    ...
    try:
        ...
        if len(parts)==5 and parts[0]=="mmr" and parts[1]=="player" and parts[3]=="next":
            uuid=parts[2].lower();platform=parts[4]
            if platform not in ("1.8","1.21"):raise ValueError("unsupported platform")
            return {"ok":True,"target":competitive.get_mmr_target(c,uuid,platform)}
    ...
    try:
        ...
        if len(parts)>=2 and parts[0]=="seasons":
            ...
            if len(parts)==4 and parts[2]=="player":
                uuid=parts[3].lower();row=c.execute("SELECT 1 FROM season_players WHERE season_id=? AND lower(uuid)=?",(sid,uuid)).fetchone();return {"ok":True,"seasonId":sid,"player":next((p for p in competitive.season_summary(c,sid)["players"] if p["uuid"].lower()==uuid),None)} if row else {"ok":True,"seasonId":sid,"player":None}
    ...
    def do_GET(self):
        ...
        try:
            if len(parts)==4 and parts[:3]==["api","v1","challenge"]:
                platform=parts[3]
                if platform not in ("1.8","1.21"):raise ValueError("unsupported platform")
                comp=CREATE_COMPETITION(platform);validate_mode(platform,comp["mode"]);send_json(self,200,{"ok":True,"platform":platform,"week":comp["week_key"],"number":comp["number"],"mode":comp["mode"],"pattern":int(comp["pattern"]),"kit":comp["kit"],"start":comp["start_ts"],"end":comp["end_ts"],"status":comp["status"]});return
            if len(parts)==5 and parts[:3]==["api","v1","challenge"] and parts[4]=="leaderboard":
                platform=parts[3]
                if platform not in ("1.8","1.21"):raise ValueError("unsupported platform")
                comp=CREATE_COMPETITION(platform);validate_mode(platform,comp["mode"]);rows=COMPETITION_ROWS(comp,10);send_json(self,200,{"ok":True,"platform":platform,"week":comp["week_key"],"number":comp["number"],"mode":comp["mode"],"pattern":int(comp["pattern"]),"kit":comp["kit"],"start":comp["start_ts"],"end":comp["end_ts"],"status":comp["status"],"rows":[{"name":n,"stage":int(s),"timeMs":int(t)} for n,s,t in rows]});return
            if len(parts) in (6,7) and parts[:3]==["api","v1","leaderboard"]:
                platform,mode=parts[3],validate_mode(parts[3],parts[4])
            ...
            if len(parts)==6 and parts[:3]==["api","v1","pb"]:
                platform,mode,uuid=parts[3],validate_mode(parts[3],parts[4]),parts[5]
                c=DB();retired_mode_cleanup(c);rows=c.execute("SELECT pattern,kit,stage,time_ms FROM runs WHERE platform=? AND mode=? AND uuid=? ORDER BY pattern ASC,stage DESC,time_ms ASC,kit ASC",(platform,mode,uuid.lower())).fetchall();c.close();send_json(self,200,{"ok":True,"platform":platform,"mode":mode,"uuid":uuid.lower(),"rows":[{"pattern":int(p),"kit":k,"stage":int(s),"timeMs":int(t)} for p,k,s,t in rows]});return
            if len(parts)>=3 and parts[:2]==["api","v1"]:
                if parts[2]=="tournament":
                    if parts[3:]==["leaderboard"]:result=competitive_get(["tournament","leaderboard"]);send_json(self,200,result);return
    ...
    def do_POST(self):
        ...
        try:
            ...
            if path=="/api/v1/matches":
                ...
                tournament_id=payload.get("tournamentId");tournament_match_id=payload.get("tournamentMatchId");tournament_game_number=payload.get("tournamentGameNumber")
                ...
                c=DB();retired_mode_cleanup(c);competitive.ensure_schema(c);accepted=competitive.record_match(c,{"id":str(payload["matchId"])[:128],"platform":platform,"mode":mode,"pattern":int(payload["pattern"]),"kit":str(payload["kit"])[:64],"started_at":int(payload["startedAt"]),"ended_at":int(payload["endedAt"]),"tournament_id":tournament_id,"tournament_match_id":tournament_match_id,"tournament_game_number":tournament_game_number},players);c.close();send_json(self,200,{"ok":True,"accepted":bool(accepted),"matchId":str(payload["matchId"])});return
            send_json(self,404,{"ok":False,"error":"not_found"})
