...
[CmdletBinding()]
param([string]$Version='0.1.0-1.21',[string]$Note='Initial 1.21 Solo release.')
...
foreach ($rel in $updateable) { Add-File $rel (Join-Path $here $rel) }
$maps = Join-Path $here 'maps'
foreach ($map in @('mm_colombia','mm_sandycoast','mm_siberian','mm_swampland','mm_tesorohundido','mm_volcano')) {
    $root = Join-Path $maps $map
    if (-not (Test-Path $root)) { throw "Converted map missing: $map" }
...
[IO.File]::WriteAllText((Join-Path $here 'version.json'),($manifest|ConvertTo-Json -Depth 10),$utf8)
