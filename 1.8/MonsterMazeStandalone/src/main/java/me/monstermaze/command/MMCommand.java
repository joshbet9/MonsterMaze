...
public class MMCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
            ...
            sender.sendMessage(ChatColor.AQUA + "Next pattern: " + ChatColor.WHITE + formatPattern(gm.getMazeGenerator().getForcedPattern()));
            return true;
        }
        ...
        if (sub.equals("kit") || sub.equals("kits")) {
            ...
            if (sub.equals("kits")) { showKits(p, gm); return true; }
            if (args.length < 2) {
                if (gm.isLive()) { sender.sendMessage(ChatColor.RED + "Can't change kit mid-game."); return true; }
                gm.getKitManager().openSelector(p); return true;
            }
        ...
        switch (sub) {
            case "void": case "voidworld": case "lobby":
                if (!(sender instanceof Player)) { sender.sendMessage(ChatColor.RED + "Players only."); return true; }
                plugin.getVoidWorlds().sendToVoid((Player) sender); break;
            case "start":
                ...
                if (sender instanceof Player) gm.startGame(((Player) sender).getLocation()); else gm.startGame();
                sender.sendMessage(ChatColor.GREEN + "Starting Monster Maze..."); break;
