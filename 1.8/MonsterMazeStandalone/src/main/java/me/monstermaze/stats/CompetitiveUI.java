...
public final class CompetitiveUI {
    private static final Pattern NUM = Pattern.compile("\\\"%s\\\":(-?\\d+(?:\\.\\d+)?)");
    ...
    public void showStats(final Player player) {
        if (!plugin.getBackendClient().isEnabled()) {
            player.sendMessage(ChatColor.YELLOW + "Competitive stats are unavailable while the backend is disabled.");
            return;
        }
        new BukkitRunnable() {
            @Override public void run() {
                try {
                    ...
                    Bukkit.getScheduler().runTask(plugin, new Runnable() {
                        @Override public void run() { if (player.isOnline()) sendLines(player, message); }
                    });
                } catch (final Exception e) {
                    Bukkit.getScheduler().runTask(plugin, new Runnable() {
                        @Override public void run() { if (player.isOnline()) player.sendMessage(ChatColor.RED + "Could not load competitive stats: " + e.getMessage()); }
                    });
                }
            }
    ...
    public void showLeaderboard(final Player player, final String kind) {
        ...
        if (!(k.equals("mmcl") || k.equals("mmr") || k.equals("elo") || k.equals("weekly") || k.equals("tournament"))) {
            player.sendMessage(ChatColor.RED + "Usage: /mm clb <mmcl|mmr|elo|weekly|tournament>");
            return;
        }
        if (!plugin.getBackendClient().isEnabled()) {
            player.sendMessage(ChatColor.YELLOW + "Competitive leaderboards are unavailable while the backend is disabled.");
            return;
        }
        new BukkitRunnable() {
            @Override public void run() {
                try {
                    String json = plugin.getBackendClient().get("/api/v1/" + k + "/leaderboard");
                    final String text = formatLeaderboard(k, json);
                    Bukkit.getScheduler().runTask(plugin, new Runnable() {
                        @Override public void run() { if (player.isOnline()) sendLines(player, text); }
                    });
                } catch (final Exception e) {
                    Bukkit.getScheduler().runTask(plugin, new Runnable() {
                        @Override public void run() { if (player.isOnline()) player.sendMessage(ChatColor.RED + "Could not load competitive leaderboard: " + e.getMessage()); }
                    });
                }
            }
        }.runTaskAsynchronously(plugin);
    ...
    private String formatStats(String mmcl, String mmr, String season) {
        double elo = number(mmcl, "elo");
        double weekly = number(mmcl, "weeklyPoints");
        double tournament = number(mmcl, "tournamentPoints");
        double ec = number(mmcl, "eloComponent");
        double wc = number(mmcl, "weeklyComponent");
        double tc = number(mmcl, "tournamentComponent");
        double score = number(mmcl, "mmcl");
        double permanent = number(mmr, "mmr");
        double seasonNumber = number(season, "number");
        StringBuilder s = new StringBuilder();
        s.append(ChatColor.GOLD).append("=== Monster Maze Competitive ===\n");
    ...
    private String formatLeaderboard(String kind, String json) {
        StringBuilder s = new StringBuilder();
        s.append(ChatColor.GOLD).append("=== ").append(kind.toUpperCase()).append(" Leaderboard ===\n");
        Matcher m = ROW.matcher(json == null ? "" : json);
        int rank = 1;
        while (m.find() && rank <= 10) {
            s.append(ChatColor.GRAY).append("#").append(rank++).append(" ").append(ChatColor.WHITE).append(unescape(m.group(1))).append(ChatColor.DARK_GRAY).append(" — ").append(ChatColor.GOLD).append(fmt(Double.parseDouble(m.group(2)))).append("\n");
        }
    ...
    private static double number(String json, String key) { if (json == null) return -1; Matcher m = Pattern.compile(String.format(NUM.pattern(), Pattern.quote(key))).matcher(json); return m.find() ? Double.parseDouble(m.group(1)) : -1; }
