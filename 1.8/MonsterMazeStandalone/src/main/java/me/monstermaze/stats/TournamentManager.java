...
/** Hosted-server tournament cache and game coordinator. Inactive when the backend is not configured. */
public final class TournamentManager {
    private final MonsterMazePlugin plugin;
    private final BackendClient backend;
    private volatile Tournament tournament;
    private final Map<String,Match> playerMatches = new HashMap<String,Match>();
    private volatile List<Row> leaderboard = new ArrayList<Row>();
    private volatile boolean startingTournamentGame;

    public TournamentManager(MonsterMazePlugin plugin) {
        this.plugin = plugin;
        this.backend = plugin.getBackendClient();
        if (backend != null && backend.isEnabled()) {
            Bukkit.getScheduler().runTaskLater(plugin, new Runnable() { public void run() { refresh(); } }, 60L);
            Bukkit.getScheduler().runTaskTimer(plugin, new Runnable() { public void run() { refresh(); } }, 600L, 600L);
        }
    }
    ...
    public void refresh() {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
            public void run() {
                try {
                    tournament = parseTournament(backend.get("/api/v1/tournament/current"));
                    leaderboard = parseLeaderboard(backend.get("/api/v1/tournament/leaderboard"));
                    for (Player p : Bukkit.getOnlinePlayers()) refreshPlayer(p);
                    Bukkit.getScheduler().runTaskLater(plugin, new Runnable() { public void run() { tryStartTournamentMatch(); } }, 40L);
                } catch (Exception e) {
    ...
    private void refreshPlayer(final Player p) {
        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() {
            public void run() {
                try {
                    Match m = parseMatch(backend.get("/api/v1/tournament/player/" + p.getUniqueId()));
                    synchronized (playerMatches) {
                        String k = p.getUniqueId().toString().toLowerCase();
                        if (m == null) playerMatches.remove(k); else playerMatches.put(k, m);
                    }
    ...
    /**
     * Starts a tournament game only when exactly the two backend-assigned players
     * are the non-spectating players online. This lets the existing GameManager
     * and CompetitiveMatchTracker run the normal game/result pipeline while
     * preventing an unrelated lobby player from entering the series.
     */
    private void tryStartTournamentMatch() {
        if (!isEnabled() || startingTournamentGame) return;
        Tournament t = tournament;
        if (t == null || !"bracket".equalsIgnoreCase(t.status)) return;
        if (plugin.getGameManager() == null || plugin.getGameManager().getState() != me.monstermaze.game.GameState.IDLE) return;

        List<Player> candidates = new ArrayList<Player>();
        for (Player p : Bukkit.getOnlinePlayers()) {
            if (p.getGameMode() != GameMode.SPECTATOR) candidates.add(p);
        }
        if (candidates.size() != 2) return;

        Match a = matchFor(candidates.get(0));
        Match b = matchFor(candidates.get(1));
        if (a == null || b == null) return;
        if (!samePair(a, b)) return;
        if (!isPlayer(a, candidates.get(0)) || !isPlayer(a, candidates.get(1))) return;

        final Player p1 = candidates.get(0);
        final Player p2 = candidates.get(1);
        startingTournamentGame = true;
        p1.sendMessage(ChatColor.GOLD + "=== Tournament Game " + a.nextGame + "/3 ===");
        p2.sendMessage(ChatColor.GOLD + "=== Tournament Game " + a.nextGame + "/3 ===");
        p1.sendMessage(ChatColor.YELLOW + "Opponent: " + ChatColor.WHITE + nameFor(a, p1));
        p2.sendMessage(ChatColor.YELLOW + "Opponent: " + ChatColor.WHITE + nameFor(a, p2));
        p1.sendMessage(ChatColor.GREEN + "Your tournament match is starting!");
        p2.sendMessage(ChatColor.GREEN + "Your tournament match is starting!");
        Bukkit.getScheduler().runTask(plugin, new Runnable() {
            public void run() {
                try {
                    if (plugin.getGameManager().getState() == me.monstermaze.game.GameState.IDLE
                            && p1.isOnline() && p2.isOnline()) {
                        plugin.getGameManager().startGame();
                    }
                } finally {
                    startingTournamentGame = false;
                }
            }
        });
    }

    private static boolean samePair(Match a, Match b) {
        return (a.p1.equalsIgnoreCase(b.p1) && a.p2.equalsIgnoreCase(b.p2))
                || (a.p1.equalsIgnoreCase(b.p2) && a.p2.equalsIgnoreCase(b.p1));
    }
    private static boolean isPlayer(Match m, Player p) {
        String u = p.getUniqueId().toString();
        return m.p1.equalsIgnoreCase(u) || m.p2.equalsIgnoreCase(u);
    }
    private static String nameFor(Match m, Player p) {
        return m.p1.equalsIgnoreCase(p.getUniqueId().toString()) ? m.p2Name : m.p1Name;
    }
    private Match matchFor(Player p) {
        synchronized (playerMatches) { return playerMatches.get(p.getUniqueId().toString().toLowerCase()); }
    }

    public void show(Player p, boolean lbOnly) {
        ...
        if (!lbOnly) {
            ...
            Match m = matchFor(p);
            if (m != null) {
                boolean first = m.p1.equalsIgnoreCase(p.getUniqueId().toString());
                p.sendMessage(ChatColor.YELLOW + "Your match: " + ChatColor.WHITE + (first ? m.p1Name : m.p2Name) + " vs " + (first ? m.p2Name : m.p1Name));
        ...
        if (leaderboard.isEmpty()) p.sendMessage(ChatColor.GRAY + "No tournament points recorded yet.");
        else { int rank = 1; for (Row r : leaderboard) { p.sendMessage(ChatColor.GRAY + "#" + rank + " " + ChatColor.WHITE + r.name + ChatColor.DARK_GRAY + " — " + ChatColor.GOLD + r.points + " pts"); rank++; } }
        p.sendMessage("");
    }
    ...
    public void showMatch(Player p) {
        ...
        Match m = matchFor(p);
        if (m == null) { p.sendMessage(ChatColor.YELLOW + "You do not currently have a playable tournament match."); refreshPlayer(p); return; }
        boolean first = m.p1.equalsIgnoreCase(p.getUniqueId().toString());
        p.sendMessage(ChatColor.GOLD + "=== Your Tournament Match ===");
        p.sendMessage(ChatColor.WHITE + (first ? m.p1Name : m.p2Name) + ChatColor.GRAY + " vs " + ChatColor.WHITE + (first ? m.p2Name : m.p1Name));
