...
public class MonsterManager {
    ...
    private void bump() {
        ...
        for (Player player : players) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                away.setY(0);
                if (away.lengthSquared() <= 1e-6) away = new Vector(1, 0, 0);
                UtilAction.velocity(player, away.normalize().multiply(-1), 1, true, 0.75, 1.2, 2, true);
                player.damage(4.0, ent);
                Bukkit.getPluginManager().callEvent(new MonsterBumpPlayerEvent(player));
                break;
            }
        }
    ...
    private boolean canBump(Player p) {
        Long last = bumpCooldown.get(p.getUniqueId());
