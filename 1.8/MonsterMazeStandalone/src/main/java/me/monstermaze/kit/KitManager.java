...
public class KitManager implements Listener {
    ...
    public void applyKit(Player player) {
        ...
        giveVisibilityDye(player, 10, ChatColor.GREEN + "Viewing: Players Visible");
    ...
    // -------------------- Jump lock (amp -10) + Jumper jumpEvent --------------------

    /**
     * MonsterMaze.PotionEffects: Jump amp -10 unless Jumper with feathers remaining.
     */
    public void tickJumpLock() {
        ...
        for (Player p : game.getAlivePlayers()) {
            ...
            if (allowJump) {
                // PERF: a Jumper never holds the jump-block effect, so only send the
                // remove packet the once when it is actually present.
                if (p.hasPotionEffect(PotionEffectType.JUMP)) {
                    p.removePotionEffect(PotionEffectType.JUMP);
                }
            } else {
                PotionEffect cur = null;
                ...
                if (cur == null || cur.getAmplifier() != jumpBlockAmp()) {
                    // Match source ("No jumping" Jump, duration 9999999): apply once, apply forever,
                    // never let it tick down and get silently refreshed (that refresh window let a
                    // spammed jump slip through as real).
                    p.addPotionEffect(new PotionEffect(PotionEffectType.JUMP, 9999999, jumpBlockAmp(), true, false), true);
                }
            }
        }
    }

    /** Amplifier used to block jumping: -10 ("No jumping") pins players to the ground so
     *  only Jumper-with-charges can jump. (This also allows the base 'speeding' jump-spam
     *  boost, which is intentional in Original/Speed/Modern.) */
    private int jumpBlockAmp() {
        return -10;
    }
    ...
    private void jumpEvent() {
        ...
        for (Player p : game.getAlivePlayers()) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                under.getWorld().playEffect(under.getLocation(), Effect.STEP_SOUND, under.getTypeId());
                for (int dx = -1; dx <= 1; dx++) {
                    for (int dz = -1; dz <= 1; dz++) {
                        ...
                        //noinspection deprecation
                        adj.getWorld().playEffect(adj.getLocation(), Effect.STEP_SOUND, adj.getTypeId());
                    }
                }
                break;
            }
    ...
    public void onSnowballHit(EntityDamageByEntityEvent event) {
        ...
        target.getWorld().playSound(target.getLocation(), Sound.SLIME_WALK, 1f, 1.2f);
    }
        ...
        if (first) {
            ...
            if (getKit(player) == KitType.BODY_BUILDER) {
                ...
                if (max < 30.0) {
                    ...
                    player.getWorld().playEffect(player.getEyeLocation().add(0, 0.5, 0), Effect.HEART, 0);
                }
            }
        } else {
            heal(player, 2.0);
    ...
    public void onDyeToggle(PlayerInteractEvent event) {
        ...
        short[] order = { 10, 8, 5 }; // lime, grey, purple
        String[] names = {
        ...
        int idx = 0;
        for (int i = 0; i < order.length; i++) if (order[i] == current) { idx = i; break; }
    ...
    private void giveVisibilityDye(Player player, int dyeData, String name) {
        player.getInventory().setItem(7, namedAmount(Material.INK_SACK, 1, name, (short) dyeData));
        player.updateInventory();
    }

    /** Record the observer's chosen view of others and re-render everyone's view. */
    private void applyVisibilityState(Player observer, short dyeData) {
        VisMode mode;
        if (dyeData == 8) mode = VisMode.INVISIBLE;
        else if (dyeData == 5) mode = VisMode.TRANSPARENT;
    ...
    public void onInvClick(InventoryClickEvent event) {
        ...
        if (event.getInventory() != null && event.getInventory().getTitle() != null
                && event.getInventory().getTitle().equals(KitGUI.TITLE)) {
            return;
        }
        event.setCancelled(true);
    }
    ...
    private static ItemStack namedAmount(Material mat, int amount, String name) {
        return namedAmount(mat, amount, name, (short) 0);
    }

    private static ItemStack namedAmount(Material mat, int amount, String name, short data) {
