...
public final class TournamentManager {
    ...
    public TournamentManager(MonsterMazePlugin plugin) {
        this.plugin = plugin; this.backend = plugin.getBackendClient();
    ...
    public void refresh() {
        if (!isEnabled()) return;
        Bukkit.getScheduler().runTaskAsynchronously(plugin, new Runnable() { @Override public void run() {
            try { tournament=parseTournament(backend.get("/api/v1/tournament/current")); leaderboard=parseLeaderboard(backend.get("/api/v1/tournament/leaderboard")); for(Player p:Bukkit.getOnlinePlayers()) refreshPlayer(p); }
            catch(Exception e){ plugin.getLogger().warning("Tournament sync failed: "+e.getMessage()); }
        }});
    }
    private void refreshPlayer(final Player p) { Bukkit.getScheduler().runTaskAsynchronously(plugin,new Runnable(){@Override public void run(){try{playerMatch=parseMatch(backend.get("/api/v1/tournament/player/"+p.getUniqueId()));}catch(Exception ignored){}}}); }
    public void show(Player p, boolean lbOnly) {
        ...
        Tournament t=tournament;if(t==null){p.sendMessage(ChatColor.YELLOW+"Tournament information is still loading...");refresh();return;}
        p.sendMessage("");p.sendMessage(ChatColor.GOLD+"=== Tournament #"+t.number+" — "+t.name+" ===");p.sendMessage(ChatColor.AQUA+"Status: "+ChatColor.WHITE+pretty(t.status));
        if(!lbOnly){p.sendMessage(ChatColor.AQUA+"Players: "+ChatColor.WHITE+t.players+" | Bracket: "+t.bracketSize);if(t.start!=null)p.sendMessage(ChatColor.AQUA+"Start: "+ChatColor.WHITE+formatDate(t.start));Match m=playerMatch;if(m!=null){String you=m.p1.equalsIgnoreCase(p.getUniqueId().toString())?m.p1Name:m.p2Name;String opp=m.p1.equalsIgnoreCase(p.getUniqueId().toString())?m.p2Name:m.p1Name;p.sendMessage(ChatColor.YELLOW+"Your match: "+ChatColor.WHITE+you+" vs "+opp);p.sendMessage(ChatColor.YELLOW+"Game: "+ChatColor.WHITE+m.nextGame+"/3"+ChatColor.GRAY+" — series "+m.w1+"–"+m.w2);}else p.sendMessage(ChatColor.GRAY+"You have no currently playable tournament match.");}
        p.sendMessage(ChatColor.YELLOW+"--- Tournament Points ---");if(leaderboard.isEmpty())p.sendMessage(ChatColor.GRAY+"No tournament points recorded yet.");else{int rank=1;for(Row r:leaderboard){p.sendMessage(ChatColor.GRAY+"#"+rank+" "+ChatColor.WHITE+r.name+ChatColor.DARK_GRAY+" — "+ChatColor.GOLD+r.points+" pts");rank++;}}p.sendMessage("");
    }
    public void showMatch(Player p){if(!isEnabled()){p.sendMessage(ChatColor.GRAY+"Tournament information is available on the hosted Monster Maze server.");return;}Match m=playerMatch;if(m==null){p.sendMessage(ChatColor.YELLOW+"You do not currently have a playable tournament match.");refresh();return;}boolean first=m.p1.equalsIgnoreCase(p.getUniqueId().toString());p.sendMessage(ChatColor.GOLD+"=== Your Tournament Match ===");p.sendMessage(ChatColor.WHITE+(first?m.p1Name:m.p2Name)+ChatColor.GRAY+" vs "+ChatColor.WHITE+(first?m.p2Name:m.p1Name));p.sendMessage(ChatColor.AQUA+"Game "+ChatColor.WHITE+m.nextGame+ChatColor.AQUA+"/3");p.sendMessage(ChatColor.AQUA+"Series: "+ChatColor.WHITE+m.w1+"–"+m.w2);}
    private static Tournament parseTournament(String j){if(j==null||!j.contains("\"tournament\""))return null;String o=objectAfter(j,"tournament");if(o==null||o.trim().equals("null"))return null;String n=string(o,"name"),s=string(o,"status"),st=string(o,"start");int no=integer(o,"number",0),b=integer(o,"bracketSize",0),pc=countObjects(array(o,"players"));return no<1||n==null?null:new Tournament(no,n,s,st,b,pc);}
    private static List<Row> parseLeaderboard(String j){List<Row> out=new ArrayList<Row>();if(j==null)return out;String a=array(j,"rows");if(a==null)return out;Matcher m=Pattern.compile("\\{([^{}]*)\\}").matcher(a);while(m.find()){String o=m.group(1),n=string(o,"name");int p=integer(o,"score",integer(o,"points",0));if(n!=null)out.add(new Row(n,p));}return out;}
    private static Match parseMatch(String j){if(j==null||!j.contains("\"match\""))return null;String o=objectAfter(j,"match");if(o==null||o.trim().equals("null"))return null;String a=string(o,"player1"),b=string(o,"player2");if(a==null||b==null)return null;int x=integer(o,"player1Wins",0),y=integer(o,"player2Wins",0);return new Match(a,b,a.substring(0,Math.min(8,a.length())),b.substring(0,Math.min(8,b.length())),x,y,x+y+1);}
    ...
    private static int integer(String j,String k,int f){Matcher m=Pattern.compile("\\\""+Pattern.quote(k)+"\\\"\\s*:\\s*(-?\\d+)").matcher(j);if(!m.find())return f;try{return Integer.parseInt(m.group(1));}catch(Exception e){return f;}}
