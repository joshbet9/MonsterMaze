...
public final class TournamentManager {
    private final MonsterMazePlugin plugin; private final BackendClient backend; private volatile Tournament tournament; private final Map<String,Match> playerMatches=new HashMap<String,Match>(); private volatile List<Row> leaderboard=new ArrayList<Row>();
    ...
    public void refresh(){if(!isEnabled())return;Bukkit.getScheduler().runTaskAsynchronously(plugin,new Runnable(){public void run(){try{tournament=parseTournament(backend.get("/api/v1/tournament/current"));leaderboard=parseLeaderboard(backend.get("/api/v1/tournament/leaderboard"));for(Player p:Bukkit.getOnlinePlayers())refreshPlayer(p);}catch(Exception e){plugin.getLogger().warning("Tournament sync failed: "+e.getMessage());}}});}
    private void refreshPlayer(final Player p){Bukkit.getScheduler().runTaskAsynchronously(plugin,new Runnable(){public void run(){try{Match m=parseMatch(backend.get("/api/v1/tournament/player/"+p.getUniqueId()));synchronized(playerMatches){String k=p.getUniqueId().toString().toLowerCase();if(m==null)playerMatches.remove(k);else playerMatches.put(k,m);}}catch(Exception ignored){}}});}
