...
public class KitManager implements Listener {
    ...
    public void applyKit(Player player) {
        ...
        switch (kit) {
            ...
            case SLOWBALL:
                ...
                player.getInventory().setItem(0, namedAmount(Material.SNOWBALL, 1, ChatColor.AQUA + "Slowball"));
    ...
    private void constructorSlowball() {
        for (Player p : game.getAlivePlayers()) {
            ...
            int amount = 0;
            if (slot != null && slot.getType() == Material.SNOWBALL) {
                amount = slot.getAmount();
            }
            if (amount >= 16) continue;

            if (amount <= 0) {
                p.getInventory().setItem(0, namedAmount(Material.SNOWBALL, 1, ChatColor.AQUA + "Slowball"));
            } else {
