...
public class MazeGenerator {
    ...
    public void buildLobby(Location origin) {
        ...
        for (int x = -LOBBY_R; x <= LOBBY_R; x++) {
            for (int z = -LOBBY_R; z <= LOBBY_R; z++) {
                ...
                for (int y = 0; y <= 2; y++) {
                    world.getBlockAt(cx + x, ly + y, cz + z).setType(Material.WHITE_STAINED_GLASS);
                }
            }
        }
    ...
    private void buildMazeRow(World world, int cx, int cy, int cz, int mz, MazeBlockData theme) {
        for (int mx = 0; mx < MAZE_SIZE; mx++) {
            ...
            if (isSpawn) {
                trackSet(world.getBlockAt(wx, cy - 1, wz), Material.REDSTONE_BLOCK);
            ...
            if (isBarrier) {
                for (int y = 0; y <= 2; y++) {
                    ...
                    trackSet(g, Material.CYAN_STAINED_GLASS);
    ...
    private void trackSet(Block block, Material mat) {
