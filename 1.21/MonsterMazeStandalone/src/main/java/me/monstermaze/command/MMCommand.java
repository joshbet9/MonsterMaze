...
public class MMCommand implements CommandExecutor {
    ...
    @Override public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (sub.equals("kit") || sub.equals("kits")) {
            if (!(sender instanceof Player)) { sender.sendMessage(ChatColor.RED + "Players only."); return true; }
            Player p = (Player)sender; if (sub.equals("kits")) { showKits(p, gm); return true; }
            if (args.length < 2) { if (gm.isLive()) { sender.sendMessage(ChatColor.RED + "Can't change kit mid-game."); return true; } gm.getKitManager().openSelector(p); return true; }
            KitType kit = KitType.byName(args[1]); if (kit == null && args[1].equalsIgnoreCase("body")) kit = KitType.BODY_BUILDER;
            ...
            gm.getKitManager().setKit(p, kit); return true;
        }
        if (sub.equals("pb") || sub.equals("lb") || sub.equals("leaderboard")) {
            if (!(sender instanceof Player)) { sender.sendMessage(ChatColor.RED + "Players only."); return true; }
            Player p = (Player)sender; if (sub.equals("pb")) showPB(p); else showLeaderboard(p,args); return true;
        }
        if (!sender.hasPermission("monstermaze.admin")) { sender.sendMessage(ChatColor.RED + "No permission."); return true; }
        switch (sub) {
            case "void": case "voidworld": case "lobby": if (!(sender instanceof Player)) { sender.sendMessage(ChatColor.RED + "Players only."); return true; } plugin.getVoidWorlds().sendToVoid((Player)sender); break;
            case "start": { if (gm.getState()!=GameState.IDLE && gm.getState()!=GameState.ENDING) { sender.sendMessage(ChatColor.RED+"Game already running ("+gm.getState()+")."); return true; } Integer requested=null; if(args.length>=2) try { int p=Integer.parseInt(args[1]); if(p<1||p>MazeLayouts.ALL_MAZES.length){sender.sendMessage(ChatColor.RED+"Invalid maze pattern.");return true;}requested=p-1;}catch(NumberFormatException e){sender.sendMessage(ChatColor.RED+"Invalid maze pattern.");return true;} int pattern=requested==null?forcedPattern():requested; if(sender instanceof Player){if(pattern>=0)gm.startGame(((Player)sender).getLocation(),pattern);else gm.startGame(((Player)sender).getLocation());}else{if(pattern>=0)gm.startGame(pattern);else gm.startGame();} sender.sendMessage(ChatColor.GREEN+"Starting Monster Maze"+(pattern>=0?" with Maze "+(pattern+1):" with a random maze")+"..."); break; }
            case "pattern": case "maze": if(args.length<2){sender.sendMessage(ChatColor.YELLOW+"Usage: /mm pattern <1|2|3|random>");return true;} if(gm.isRunning()){sender.sendMessage(ChatColor.RED+"Change the pattern when no game is running.");return true;} String pa=args[1].toLowerCase(); if(pa.equals("random")){setForcedPattern(-1);sender.sendMessage(ChatColor.GREEN+"Maze pattern set to random for the next game.");return true;} int pattern;try{pattern=Integer.parseInt(pa);}catch(NumberFormatException e){sender.sendMessage(ChatColor.RED+"Invalid pattern.");return true;}if(pattern<1||pattern>MazeLayouts.ALL_MAZES.length){sender.sendMessage(ChatColor.RED+"Invalid pattern.");return true;}setForcedPattern(pattern-1);sender.sendMessage(ChatColor.GREEN+"Maze pattern set to "+ChatColor.WHITE+pattern+ChatColor.GREEN+" for the next game.");break;
            case "map": case "arena": if(args.length<2){sender.sendMessage(ChatColor.RED+"Usage: /mm map <name>");return true;}if(gm.isRunning()){sender.sendMessage(ChatColor.RED+"Change the map when no game is running.");return true;}String want=args[1].toLowerCase();if(!plugin.getMapManager().isKnown(want)){sender.sendMessage(ChatColor.RED+"Unknown map '"+args[1]+"'.");return true;}if(!plugin.getMapManager().isAvailable(want)){sender.sendMessage(ChatColor.RED+"Map world '"+want+"' is not installed on this server.");return true;}if(!plugin.getMapManager().setActiveMap(want)){sender.sendMessage(ChatColor.RED+"Could not activate map '"+want+"'.");return true;}org.bukkit.Location center=plugin.getMapManager().defaultCenter();if(center==null){sender.sendMessage(ChatColor.RED+"Map world could not be loaded.");return true;}gm.getMonsterManager().setMobType(plugin.getMapManager().activeMob());gm.setCenter(center);sender.sendMessage(ChatColor.GREEN+"Map set to "+ChatColor.WHITE+want+ChatColor.GREEN+". Lobby moved; run /mm start.");break;
            case "mode": if(args.length<2){sender.sendMessage(ChatColor.RED+"Usage: /mm mode <original|modern|classic>");sender.sendMessage(ChatColor.AQUA+"Current: "+plugin.getMode().color+plugin.getMode().id);for(MazeMode m:MazeMode.values())sender.sendMessage(ChatColor.GRAY+" - "+m.color+m.id+ChatColor.GRAY+": "+m.description);return true;}if(gm.isRunning()){sender.sendMessage(ChatColor.RED+"Change the mode when no game is running.");return true;}MazeMode mode=MazeMode.byName(args[1]);if(mode==null){sender.sendMessage(ChatColor.RED+"Unknown mode. Try original, modern, or classic.");return true;}plugin.setMode(mode);gm.rerenderLeaderboardBoard();sender.sendMessage(ChatColor.GREEN+"Mode set to "+mode.color+mode.id+ChatColor.GREEN+". It will apply on the next /mm start.");break;
            case "stop": case "force": gm.forceStop();sender.sendMessage(ChatColor.GREEN+"Monster Maze force stopped.");break;
            case "setcenter": if(!(sender instanceof Player)){sender.sendMessage(ChatColor.RED+"Only players can set the center.");return true;}Player pl=(Player)sender;gm.setCenter(pl.getLocation());sender.sendMessage(ChatColor.GREEN+"Lobby box placed at "+pl.getLocation().getBlockX()+", "+pl.getLocation().getBlockY()+", "+pl.getLocation().getBlockZ());break;
            case "status": sender.sendMessage(ChatColor.AQUA+"State: "+ChatColor.WHITE+gm.getState());sender.sendMessage(ChatColor.AQUA+"Stage: "+ChatColor.WHITE+gm.getStage());sender.sendMessage(ChatColor.AQUA+"Alive: "+ChatColor.WHITE+gm.getAlivePlayers().size());sender.sendMessage(ChatColor.AQUA+"Map: "+ChatColor.WHITE+plugin.getMapManager().getActiveMap());sender.sendMessage(ChatColor.AQUA+"Next pattern: "+ChatColor.WHITE+formatPattern(forcedPattern()));break;
